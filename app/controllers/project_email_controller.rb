...
class ProjectEmailController < ApplicationController
  ...
  def save
    ...
    if @email then
      if params['send'] then
        ...
        redirect_to :action => 'compose', :project_id => @email.project.id, :email_id => @email.id
      end
    else
  ...
  def send_email
    flash[:notice] = 'TODO: Send email'
    redirect_to :action => 'index', :project_id => @email.project.id
  end
    ...
    if emailp then
      @email =
        begin
          ...
          ProjectEmail.new :sender => User.current, :sent => false, :project => (Project.find emailp['project_id'])
        end

      @email.project_id = emailp['project_id']
